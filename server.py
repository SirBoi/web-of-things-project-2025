...
def on_message(client, userdata, msg):
    try:
        ...
        print(f"> MQTT parse/save error on topic {msg.topic}: {e}")
...
def server_console_task():
    print("> Server console ready. Type: <target> <command...>  (example: dpir1 on)")
...
def get_camera():
    ...
    with _cam_lock:
        if _cam is None:
            _cam = _open_camera()
            print(f"> CAM opened? {_cam.isOpened()}")
            ...
            try:
                ...
                pass
            _cam = _open_camera()
            print(f"> CAM reopened? {_cam.isOpened()}")
        return _cam

def gen_frames():
    print(">>> gen_frames() START")
    fail = 0
    while True:
        cam = get_camera()
        ok, frame = cam.read()
            ...
            if fail >= 30:
                with _cam_lock:
                    try:
                        ...
                        pass
                    globals()["_cam"] = None
                print("> CAM read failed repeatedly, reopening...")
                fail = 0
            time.sleep(0.05)
            continue
